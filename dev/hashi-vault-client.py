...
VAULT_ID_SECRET_MAP = {"bmrc": "bmrc_staging_ansible_vault_password"}
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Path to a CA bundle to use for verification. Enables certificate verification."
        ),
    )
    parser.add_argument(
        "-s",
    ...
    parser.add_argument("--vault-id", required=True, help=("secret name"))
    ...
    path = f"{args.secrets_root_path}/{vault_id}"
    try:
        current = client.read(path)
    except hvac.exceptions.Forbidden as e:
        print(f"E: unable to get ansible-vault password: {e}", file=sys.stderr)
        return 1
